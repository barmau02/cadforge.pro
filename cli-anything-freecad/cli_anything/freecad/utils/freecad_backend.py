...
def find_freecad(gui_required: bool = False) -> str:
    ...
    try:
        ...
        if audit_enabled():
            return validate_frecad_executable(os.environ.get("FREECAD_PATH"))
    except ImportError:
        pass
...
def run_macro(
    ...
    # PF-SEC: only temp or allowlisted macro paths
    try:
        from cli_anything.freecad.security import audit_enabled, validate_temp_script

        if audit_enabled():
            validate_temp_script(script_path)
    except ImportError:
        pass

    safe_timeout = timeout
    try:
        from cli_anything.freecad.security import PF_MAX_TIMEOUT

        safe_timeout = min(timeout, PF_MAX_TIMEOUT)
    except ImportError:
        pass
...
def export_headless(
    ...
    RuntimeError
        ...
        file is not created.
    """
    # PF-SEC: exports must stay under batch allowlist
    try:
        from cli_anything.freecad.security import audit_enabled, validate_write_path

        if audit_enabled():
            output_path = str(validate_write_path(output_path, label="export output"))
        else:
